...
if __name__ == '__main__':
    ...
    if opt.epoch != 0:
        ...
        if cuda:
            ...
            G1.load_state_dict(torch.load("saved_models/%s/G1_%d.pth" % (opt.model_name, opt.epoch), map_location=torch.device('cpu')))
            G2.load_state_dict(torch.load("saved_models/%s/G2_%d.pth" % (opt.model_name, opt.epoch), map_location=torch.device('cpu')))
            D1.load_state_dict(torch.load("saved_models/%s/D1_%d.pth" % (opt.model_name, opt.epoch), map_location=torch.device('cpu')))
            D2.load_state_dict(torch.load("saved_models/%s/D2_%d.pth" % (opt.model_name, opt.epoch), map_location=torch.device('cpu')))
    else:
        # Initialize weights
